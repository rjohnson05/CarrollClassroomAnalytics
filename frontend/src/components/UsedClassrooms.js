...
export default function UsedClassrooms() {
                    ...
                    {classroomDropdownStatus && classroomDropdownStatus[classroom] ? data.map((course) => (
                        <div className="course" key={course}>
                            ...
                            <p className="course-seats"><b>
                                Seats
                                Utilized: </b> {(course[2] > 0) ? course[3] + ' Students / ' + course[2] + ' Seats' : "N/A"}
                            </p>
                        </div>)) : <div></div>}
                </div>
            </div>)))
                    ...
                    <div
                        ...
                        {classroomDropdownStatus && classroomDropdownStatus[classroom] ? data.map((course) => (
                            <div className="course" key={course}>
                                <p className="course-title">{course[0]}</p>
                                <p className="course-instructor"><b>Instructor: </b>{course[1]}</p>
                                <p className="course-seats"><b>Seats
                                    Occupied: </b>{(course[2] > 0) ? (course[3]) + ' students / ' + course[2] + " seats" : "N/A"}
                                </p>
                            </div>)) : <div></div>}
                    </div>
                </div>))
    ...
    return (<div>
        ...
        {startTime === '06:00' ? <div></div> : <Link className="back-button" to={{
            pathname: `/used_classrooms`,
            search: `?day=${day}&buildingList=${buildingList}&startTime=${pastStartTime}&endTime=${startTime}`
        }}>
            <NavigateBeforeIcon/>
        ...
        {endTime === '23:59' ? <div></div> : <Link className="next-button" to={{
            pathname: `/used_classrooms`,
            search: `?day=${day}&buildingList=${buildingList}&startTime=${endTime}&endTime=${nextEndTime}`
        }}>
            <NavigateNextIcon/>
            <p>Next Time Block</p>
        </Link>
